...
get_networkdata_intact <- function(species = "taxid:9606(human)|taxid:9606(Homo sapiens)",
  ...
  if (!(species %in% list_species_intact)) { # if species is not in the list
    stop(
      "Species not found as specified by intact,",
      "please check some valid entries by running `list_species_intact`"
  ...
  if (get_annotation && !is.na(annotation_db)) {
    db_intact_anno_df <- get_annotation_intact(
    ...
    if (add_annotation) {
      db_intact_ppi_anno_df <- add_annotation_intact(
...
list_db_annotationdbi_intact <- c(
  "org.At.tair.db",
  "org.Bt.eg.db",
  "org.Ce.eg.db",
  "org.Cf.eg.db",
...
intact_db_annotations <- data.frame(
  species = list_common_species_intact,
