...
get_networkdata_hippie <- function(species,
  ...
  if (get_annotation) {
    if (!(species %in% list_species_hippie)) { # if species is not in the list
      stop(
        "Species not in `list_common_species_hippie`!",
...
hippie_db_annotations <- data.frame(
  species = list_species_hippie,
...
#' )
#'
#' db_hippie_graph <- build_graph_hippie(
#'   graph_data = db_hippie_df,
